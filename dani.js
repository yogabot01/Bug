...
║🎭 Prefix :MULTI
│▬▭「 INFO OWNER ダフ️ 」▭▬
[ 🇬🇧 Please ask the owner for a password first so you can use the VynaBotz ]
[ 🇲🇨 Silahkan Meminta Password Kepada Owner Terlebih Dahulu Agar Bisa Menggunakan Bot VynaBotz 」
 `
}
...
case "jadianime2": case "drawme":
				{
					...
					const img = await haikal.downloadAndSaveMediaMessage(q)

					if (!img)
						return reply(
							`Kirim gambar dengan caption atau reply ${prefix + command}`
						)
					const { uptotelegra } = require("./baseikal/lib/uploader")

					const IMAGE_URL = await uptotelegra(img)

					const APIKEY = "6db1a278210ca11a593ef055";
					const API_URL = `https://api.lolhuman.xyz/api/imagetoanime?apikey=${APIKEY}&img=${IMAGE_URL}`
...
case "jadianime2": case "drawme":
        ...
        haikal.sendMessage(m.chat, { image: { url: `https://api.itsrose.site/image/anime/diffusion?prompt=${q}&apikey=${itsrose}` }, caption: `*© Created By ${botname}*` }, { quoted: m })
        }
        break
        case "aidraw": case "stabledif": {
        const itsrose = "6db1a278210ca11a593ef055"
        if (!q) return m.reply("Textnya Mana???")
        haikal.sendMessage(m.chat, { image: { url: `https://api.itsrose.site/image/stable/diffusion?prompt=${q}&apikey=${itsrose}` }, caption: `*© Created By ${botname}*` }, { quoted: m })
        }
        break
    case "arcane":
   	case "art":
